   ...
   public function plantilla(){
	...
	public function enlacesPaginasController(){
      ...
      // le pide al modelo y que conecte con :: al método y asi heredo la clase y sus metodos y atributos..
	  	$pagina = new Paginas();
		$respuesta = $pagina->enlacesPaginasModel($enlacesController);
		require $respuesta;
...
// funcion para devolver todas las reservas.
	 	public function getReservasController(){
	 		...
	 		$hoy = date('Y-m-d'); 
		$reservas = new Datos;
 		$respuesta = $reservas->getReservasModel('reservas');
 			# code...
 	...
 	public function agregarReservaController(){
		...
		$reservas = new Datos;
 		$respuesta= $reservas->agregarReservasModel($datosController,'reservas');
   ...
   public function deleteReservasController(){
		...
		$reservas = new Datos;
   	 	$respuesta = $reservas->deleteReservaModel($datosController,'reservas');
   	 	if ($respuesta == 'success') {
...
//cantidad de reservas
   public function totalReservasController(){
	$reservas = new Datos;
	$respuesta = $reservas->totalReservasModel('reservas');
	foreach ($respuesta as $key ) {
     ...
     public function editarReservasController(){
		...
		$reservas = new Datos;
	    $respuesta = $reservas->editarReservasModel($datosController, 'reservas');
    ...
    public function actualizarReservasController(){
		...
		$reservas = new Datos;
    	$respuesta= $reservas->actualizarReservasModel($datosController,'reservas');	
    	  	if ($respuesta == 'success') {
